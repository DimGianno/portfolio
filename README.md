...
- **Credentials:** Credential details and verification links are configured in `data/credentials.ts`. Official issuer artwork is stored in `public/credentials/` and displayed in an automatic, swipeable home-page carousel with manual navigation, play/pause control, position feedback, and reduced-motion support.
